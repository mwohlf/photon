...
public class Model2WorldMatrixValue extends Matrix4fValue {
	...
	public void accept(IShaderProgram shader) {
		...
		m3 = calculateNormalMatrix(matrix, m3);
		...
		location = shader.getUniformLocation(ShaderParser.UNIFORM_NORMAL_MTX);
		if (location != null) {
			LOGGER.debug("setting matrix uniform '{}' location is '{}', values are '{}'", ShaderParser.UNIFORM_NORMAL_MTX, location, Arrays.toString(floatArray));
